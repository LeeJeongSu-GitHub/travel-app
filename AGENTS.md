...
3. If the request is research-only, return the `travel-research.v1` JSON packet described in `.agents/skills/travel-map-builder/references/research-packet.md` and do not edit the app. If app generation is requested, create `travel/<destination-slug>/` with its entrypoint and data, then use the shared `src/travel-ui/` components. The Pages build preserves the public `/<destination-slug>/` URL, so do not add new root-level destination folders.
