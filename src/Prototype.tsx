...
type MenuItem = { name: string; nameKo?: string; price: string; note?: string };
...
function PlaceDetailSheet({ place, day, open, onClose, note, onNoteChange, completed, favorite, onToggleComplete, onToggleFavorite }: { place: Place | null; day: TripDay | undefined; open: boolean; onClose: () => void; note: string; onNoteChange: (note: string) => void; completed: boolean; favorite: boolean; onToggleComplete: () => void; onToggleFavorite: () => void }) {
  ...
  const menuItems = (place.menu ?? []).map((item) => item.nameKo ? { ...item, name: `${item.name}\n${item.nameKo}` } : item);
...
function ScheduleView({ activeDay, selectedPlace, selectedPlaceId, showAlternatives, setShowAlternatives, onSelectPlace, onToggleComplete, onToggleFavorite, completedIds, favoriteIds, userLocation, onUserLocation, onDayChange }: { activeDay: TripDay; selectedPlace: Place | null; selectedPlaceId: string | null; showAlternatives: boolean; setShowAlternatives: (show: boolean) => void; onSelectPlace: (place: MapPlace) => void; onToggleComplete: (id: string) => void; onToggleFavorite: (id: string) => void; completedIds: string[]; favoriteIds: string[]; userLocation: Coordinate | null; onUserLocation: (location: Coordinate) => void; onDayChange: (day: number) => void }) {
  ...
  return <main className="schedule-view"><section className="day-intro"><div className="day-intro-copy"><span className="eyebrow">DAY {activeDay.dayNumber} · {activeDay.dayOfMonth}일</span><h1>{activeDay.title}</h1><p>{activeDay.city}</p></div><label className="alternative-toggle"><input type="checkbox" checked={showAlternatives} onChange={(event) => setShowAlternatives(event.target.checked)} /><span className="toggle-track" /><span>대체 후보 보기</span></label></section><TripMap places={primaryPlaces.map(withDay)} routePlaces={primaryPlaces.map(withDay)} selectedPlace={selectedPlace} onSelect={onSelectPlace} userLocation={userLocation} onUserLocation={onUserLocation} /><DayTabs selectedDay={activeDay.dayOfMonth} onChange={onDayChange} /><section className="itinerary-section" aria-label={`${activeDay.dayOfMonth}일 일정 목록`}><div className="section-heading"><div><span className="eyebrow">{primaryPlaces.length} STOPS</span><h2>오늘의 동선</h2></div><span className="section-hint">카드를 눌러 상세 보기</span></div><div className="itinerary-list">{primaryPlaces.map((place, index) => <div key={`${place.id}-item`}><PlaceCard place={place} day={activeDay} selected={selectedPlaceId === place.id} completed={completedIds.includes(place.id)} favorite={favoriteIds.includes(place.id)} onSelect={() => onSelectPlace(withDay(place))} onToggleComplete={() => onToggleComplete(place.id)} onToggleFavorite={() => onToggleFavorite(place.id)} /><RouteConnector current={place} next={primaryPlaces[index + 1]} /></div>)}</div></section>{showAlternatives && restaurantAlternatives.length ? <section className="alternatives-section" aria-label="근처 대체 식당"><div className="section-heading"><div><span className="eyebrow">NEARBY RESTAURANTS</span><h2>근처 대체 식당</h2></div><span className="section-hint">기본 동선은 유지</span></div><p className="section-description">예약이 어렵거나 대기가 길 때, 해당 식당 주변에서 바로 바꿔 갈 수 있는 후보입니다.</p><div className="itinerary-list">{restaurantAlternatives.map((place) => <div key={`${place.id}-alternative`}><PlaceCard place={place} day={activeDay} selected={selectedPlaceId === place.id} completed={completedIds.includes(place.id)} favorite={favoriteIds.includes(place.id)} onSelect={() => onSelectPlace(withDay(place))} onToggleComplete={() => onToggleComplete(place.id)} onToggleFavorite={() => onToggleFavorite(place.id)} /></div>)}</div></section> : null}{showAlternatives && otherAlternatives.length ? <section className="alternatives-section other-alternatives" aria-label="대체 코스"><div className="section-heading"><div><span className="eyebrow">OPTIONAL ROUTES</span><h2>대체 촬영 코스</h2></div></div><p className="section-description">시간과 운영일을 확인한 뒤 기본 동선 대신 선택하세요.</p><div className="itinerary-list">{otherAlternatives.map((place) => <div key={`${place.id}-alternative`}><PlaceCard place={place} day={activeDay} selected={selectedPlaceId === place.id} completed={completedIds.includes(place.id)} favorite={favoriteIds.includes(place.id)} onSelect={() => onSelectPlace(withDay(place))} onToggleComplete={() => onToggleComplete(place.id)} onToggleFavorite={() => onToggleFavorite(place.id)} /></div>)}</div></section> : null}</main>;
}
