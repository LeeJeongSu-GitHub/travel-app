...
Read `README.md`, the nearest `AGENTS.md`/`CLAUDE.md`/`GEMINI.md`, `src/Prototype.tsx`, `src/travel-ui/components.tsx`, `src/travel-ui/types.ts`, `src/travel-ui/category.ts`, `src/travel-ui/index.ts`, `src/prototype.css`, [references/trip-data-contract.md](references/trip-data-contract.md), [references/design-system.md](references/design-system.md), [references/component-contract.md](references/component-contract.md), and [references/research-packet.md](references/research-packet.md) when research output is supplied or requested. Inspect existing `travel/<destination-slug>/trip.json` data, `page/<destination-slug>/index.html` entries, and the root travel hub first. When deployment is requested, also inspect `.github/workflows/deploy-pages.yml` and `vite.config.ts`. Do not overwrite another destination when the user gives a new trip. Keep `.agents/skills/travel-map-builder/` canonical; `.claude/skills/travel-map-builder/` and `.gemini/skills/travel-map-builder/` are lightweight pointers to it.

## Build the destination

1. Normalize the plan into `travel/<destination-slug>/trip.json`. Preserve dates, city order, lodging, planned order, and the user's priorities. Use stable kebab-case place IDs and numeric day/place order.
2. Add `page/<destination-slug>/index.html` as the deployable page entry and add one card to the root travel hub. Keep destination facts in `travel/<destination-slug>/trip.json`. The Pages build materializes the `page/` entry at the existing `/<destination-slug>/` public path, so do not create a new root-level destination folder. Keep shared UI and runtime in `src/`; use `src/travel-ui/components.tsx` and `src/travel-ui/types.ts` for the common UI/data boundary. Do not create a destination-specific dashboard or alternate CSS when the shared template already exists.
...
For the exact JSON fields, allowed values, uncertainty rules, and menu example, read [references/trip-data-contract.md](references/trip-data-contract.md). When the user provides a research bundle or asks for a research-only deliverable, read [references/research-packet.md](references/research-packet.md). For reusable prompts, use [prompts/travel-plan-research.md](../../../prompts/travel-plan-research.md) for the plan/research stage and [prompts/travel-app-build.md](../../../prompts/travel-app-build.md) for the one-shot app stage; the same prompts are linked from the README.
...
Use the provider-specific research prompts documented in the repository README when the user asks for a plan or research packet before app generation. The Claude Code, Gemini CLI, and GPT/Codex variants must produce the same `travel-research.v1` contract; only the web-search tool instructions differ.

1. Read the user's current request and all supplied links, documents, screenshots, and notes. Preserve the user's dates, fixed events, lodging, named places, order, budget, and constraints before adding recommendations.
2. Search each place with the city or country included. Start with the official place page, official SNS or reservation page, and exact Google Maps result. Use municipal, tourism, reservation, or reliable local sources only as cross-checks. Open the source page; do not treat a result title or snippet as proof.
3. For volatile facts, verify hours, last order, closed days, prices, reservation status, menu availability, and access information against current sources. Keep separate source URLs for separate claims when needed.
4. Resolve maps in this order: user link, exact official result, exact Google Maps place result, then a search URL. Never derive or guess coordinates from an approximate name. Use `coordinates: null`, `확인 필요`, and a reason when the pin is not verified.
5. Keep restaurant/cafe `closedDays` separate. Keep Japanese menu text in `nameJa`, add Korean `nameKo`, preserve the sourced price, and record menu image source and rights information. Reject search thumbnails, blob/data URLs, expiring URLs, and unclear image rights.
6. Return uncertainty instead of filling gaps. Use `confidence` and `needsConfirmation` for conflicts, inaccessible pages, stale information, or facts that require a visit-time check. If the work is research-only, do not edit the app or publish it.
...
- Keep current-location behavior shared across destinations: when geolocation permission is already granted, request the location automatically on both map modes; when permission is still `prompt`, request it from the `내 위치` action; show a visible loading/error state for unsupported, denied, unavailable, or timed-out requests; and fly the map to the resolved location without changing the itinerary route data.
...
After cloning, verify with `npm ci`, `npm run validate:trip`, `npm run check:runtime`, and `npm run build`. Add a trip as `travel/<destination-slug>/trip.json` plus `page/<destination-slug>/index.html`; use the shared `src/travel-ui/` components. The Pages build keeps the public URL at `/<destination-slug>/` even though the source is stored under `travel/` and `page/`. Derive Pages URLs from `GITHUB_REPOSITORY`; never hardcode the original maintainer's GitHub ID. Do not publish or push unless the user explicitly requests it.
