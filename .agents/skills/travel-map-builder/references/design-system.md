...
- 헤더 아래에 10px 여백을 두고 `DAY 1`~`DAY 4` 날짜 탭을 둡니다. 선택된 날짜는 진한 잉크색 pill 카드로 강조합니다.
- 일정 화면은 날짜 소개, 동일 높이의 실제 방문·장소 추가·대체 후보 컨트롤, 지도, 오늘의 동선 섹션, 장소 카드 순서입니다.
- 지도는 `ROUTE PREVIEW` 카드 안에서 헤더와 날짜 탭 아래에 sticky로 유지하고, 장소 목록은 그 아래에서 스크롤합니다. sticky `top` 값은 고정 숫자를 따로 추정하지 말고 실제 헤더 box 높이와 일치시켜 헤더·DAY 탭·지도가 겹치지 않게 합니다.
...
- 393~430px 모바일 화면에서 헤더·DAY 탭·sticky 지도·첫 카드·하단 내비가 서로 가리지 않습니다. 헤더 하단과 DAY 탭 상단은 10px, DAY 탭 하단과 지도 상단은 일정한 여백을 유지해야 합니다.
