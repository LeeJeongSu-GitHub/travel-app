...
- 지도는 `ROUTE PREVIEW` 카드 안에서 헤더 아래에 sticky로 유지하고, 장소 목록은 그 아래에서 스크롤합니다. 일정 지도는 모바일에서 Triple과 비슷한 compact 높이를 사용하며, 일정 스크롤이 시작되면 지도 이미지가 작은 고정 바로 접혀 카드 내용을 가리지 않게 합니다. sticky `top` 값은 고정 숫자를 따로 추정하지 말고 실제 헤더 box 높이와 일치시켜 헤더·지도·콘텐츠가 겹치지 않게 합니다.
...
- 393~430px 모바일 화면에서 헤더·지도 안 날짜 이동·compact sticky 지도·첫 카드·하단 내비가 서로 가리지 않습니다. 지도는 헤더 바로 아래 일정한 여백으로 시작하고, 날짜 이동 컨트롤은 지도 이미지와 마커를 가리지 않는 상단 모서리에 배치합니다. 일정 스크롤 중에는 지도 이미지가 축소 바가 되어 장소 카드 위에 겹치지 않아야 합니다.
