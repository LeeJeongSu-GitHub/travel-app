...
- 일정 화면에서 전체 폭을 차지하는 `DAY 1`~`DAY 4` 탭은 두지 않습니다. 지도 상단 안에 현재 `DAY N · 날짜`와 이전/다음 버튼을 compact하게 표시하고, 버튼으로 날짜를 이동합니다.
- 일정 화면은 날짜 소개, 동일 높이의 실제 방문·장소 추가·대체 후보 컨트롤, 지도, 오늘의 동선 섹션, 장소 카드 순서입니다.
- 지도는 `ROUTE PREVIEW` 카드 안에서 헤더 아래에 sticky로 유지하고, 장소 목록은 그 아래에서 스크롤합니다. 일정 지도는 모바일에서 Triple과 비슷한 compact 높이를 사용하며, 일정 스크롤이 시작되면 지도 이미지가 작은 고정 바로 접혀 카드 내용을 가리지 않게 합니다. 축소할 때는 확장 지도 슬롯의 높이를 유지해 스크롤 문서가 재배치되거나 튕기지 않게 합니다. sticky `top` 값은 고정 숫자를 따로 추정하지 말고 실제 헤더 box 높이와 일치시켜 헤더·지도·콘텐츠가 겹치지 않게 합니다.
...
4. 장소 카드의 제목·메타·휴무일·메모는 오른쪽 썸네일과 체크/하트 컨트롤을 침범하지 않아야 합니다. grid의 `minmax(0, 1fr)`, ellipsis, line clamp를 사용하고 모바일에서 가로 overflow를 허용하지 않습니다.
5. 지도·카드·상세 화면 모두 같은 장소 ID와 카테고리를 사용합니다. 지도 마커 숫자와 카드 순번은 같은 category color를 사용하며, 같은 좌표의 순번은 offset으로 분리합니다.
6. 메뉴는 일본어 원문과 한국어 번역을 한 항목 안에서 세로로 보여줍니다. 번역만 있는 메뉴나 일본어만 있는 메뉴를 기본 출력으로 만들지 않습니다.
7. 이미지 로드 실패는 깨진 이미지 아이콘으로 끝내지 않습니다. 안정적인 로컬/원격 이미지를 사용하고 `onError`로 지도·카테고리 fallback을 표시합니다.
...
- 393~430px 모바일 화면에서 헤더·지도 안 날짜 이동·compact sticky 지도·첫 카드·하단 내비가 서로 가리지 않습니다. 지도는 헤더 바로 아래 일정한 여백으로 시작하고, 날짜 이동 컨트롤은 지도 이미지와 마커를 가리지 않는 상단 모서리에 배치합니다. 일정 스크롤 중에는 지도 이미지가 축소 바가 되어 장소 카드 위에 겹치지 않아야 하며, 지도 축소 순간에도 스크롤 위치가 튀지 않아야 합니다.
