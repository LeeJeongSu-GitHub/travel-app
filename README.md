...
현재 기준 여행은 19일~22일 교토·고베 일정과 10월 1일~4일 제주 스포츠 투어 일정입니다. 교토·고베는 총 4일·40개 장소이며, 두 여행의 정본 데이터는 `travel/`, 페이지 엔트리는 `page/` 아래에 보관하고 빌드가 기존 공개 URL 경로를 유지합니다.
...
- 제주 10월 스포츠 투어는 첨부 문서의 3박 4일 일정과 2026 TransJeju by UTMB Jeju60K 공식 레이스 정보를 별도 목적지 데이터로 추가했습니다. 남원 숙소명·정확한 핀, 일부 장소의 최신 운영시간·휴무·가격은 앱에 `확인 필요`로 남겼습니다.
- DAY 1에 교토 숙소에서 짐을 푼 뒤 이동하는 `가모강·시조대교 야경`을 5번째 장소로 추가했습니다. 야외 무료 관람, 야간 산책 안내, 시조대교 좌표, Google Maps 길찾기, Wikimedia Commons 대표 이미지를 함께 기록했습니다.
