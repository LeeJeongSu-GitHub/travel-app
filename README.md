...
- 일정 화면의 지도는 헤더 아래에 compact 높이로 고정되고 지도 안에 현재 DAY·날짜와 이전/다음 이동 버튼을 표시합니다. 일정 스크롤이 시작되면 지도는 날짜 이동만 남긴 작은 고정 바로 접혀 장소 카드가 가려지지 않습니다. 지도 마커를 누르면 상세 시트를 열지 않고 해당 장소 카드로 이동·강조하고 카드 본문을 눌렀을 때만 상세 정보를 엽니다.
...
- 모바일 헤더·카드·이미지·하단 내비게이션의 잘림과 상세 시트 내부 스크롤을 반응형으로 보정했습니다. Pretendard 우선 폰트와 섹션 경계선도 공통 적용하고, 일정 상단의 방문 체크·장소 추가·대체 후보 컨트롤은 같은 높이의 한 줄 행으로 정렬합니다. 헤더의 실제 렌더링 높이를 sticky 기준과 일치시키고, 지도 안에 compact 날짜 이동을 배치해 상단이 답답하거나 일정 카드가 큰 지도에 가려지지 않도록 했습니다. 일정 스크롤이 길어지면 맨 위로 이동 버튼을 제공합니다.
...
- 헤더 → 날짜 소개/액션 → compact sticky 한글 지도(스크롤 시 날짜 이동 바로 축소) → 오늘의 동선 → 카테고리 카드 → 하단 내비게이션
...
- 헤더, 지도 안 날짜 이동, compact sticky 한글 지도, 카테고리 색상 카드, 하단 4개 메뉴, BottomSheet 상세 화면의 구조와 타이포그래피를 유지해줘.
...
<table>
  <tr>
    ...
    <td width="50%" valign="top">
      ...
      <br />헤더 아래 compact 한글 지도, 지도 안 날짜 이동, 방문 순서 마커, 실제 방문 체크, 장소 추가를 한 화면에서 사용합니다.
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
...
| 일정 | 헤더 아래 compact 고정형 지도·지도 안 날짜 이동, 스크롤 시 축소 바, 장소 목록 스크롤, 맨 위로 이동, 실제 방문 수, 대체 후보 토글, 장소 추가 |
...
중요: 현재 저장소의 교토·고베 앱 디자인을 공통 기준으로 그대로 재사용해줘. 새 여행마다 별도 초록색 dashboard나 다른 헤더/카드/CSS를 만들지 말고, 기존 `src/Prototype.tsx`, `src/prototype.css`, `MobileScroll`, `BottomSheet`를 사용해줘. 헤더·지도 안 날짜 이동·compact sticky 지도·카테고리 색상 카드·하단 내비게이션의 구조와 반응형 규칙을 유지하고, 실제 모바일 스크린샷으로 잘림·겹침을 검수해줘.
```
...
11. 모바일 실제 화면과 넓은 시뮬레이터 화면에서 헤더, 지도, 지도 안 날짜 이동,
    장소 카드, 상세 정보, 메뉴, 이미지가 잘리지 않는지 확인한다.
12. 기존 교토·고베 화면과 비교해 새 여행의 header, 지도 안 날짜 이동, compact sticky 지도,
