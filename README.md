...
- 모바일 헤더·카드·이미지·하단 내비게이션의 잘림과 상세 시트 내부 스크롤을 반응형으로 보정했습니다. Pretendard 우선 폰트와 섹션 경계선도 공통 적용하고, 일정 상단의 방문 체크·장소 추가·대체 후보 컨트롤은 같은 높이의 한 줄 행으로 정렬합니다. 헤더의 실제 렌더링 높이를 sticky 기준과 일치시키고, 지도 안에 compact 날짜 이동을 배치해 상단이 답답하거나 일정 카드가 큰 지도에 가려지지 않도록 했습니다. 지도 축소 시 확장 지도 슬롯을 유지해 모바일 스크롤 튕김을 방지하고, 웹 화면에서는 날짜 이동 버튼 사이 여백도 확보했습니다. 일정 스크롤이 길어지면 맨 위로 이동 버튼을 제공합니다.
