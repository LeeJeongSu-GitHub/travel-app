...
- 일정 화면의 지도는 헤더와 DAY 탭 아래에 고정되고 장소 목록만 스크롤됩니다. 지도 마커를 누르면 상세 시트를 열지 않고 해당 장소 카드로 이동·강조하며, 카드 본문을 눌렀을 때만 상세 정보를 엽니다.
...
- 모바일 헤더·카드·이미지·하단 내비게이션의 잘림과 상세 시트 내부 스크롤을 반응형으로 보정했습니다. Pretendard 우선 폰트와 섹션 경계선도 공통 적용하고, 일정 상단의 방문 체크·장소 추가·대체 후보 컨트롤은 같은 높이의 한 줄 행으로 정렬합니다. 헤더의 실제 렌더링 높이를 sticky 기준과 일치시켜 헤더·DAY 탭·지도 사이의 겹침도 방지하며, 헤더와 DAY 탭 사이에는 일정한 10px 호흡을 둡니다. 일정 스크롤이 길어지면 맨 위로 이동 버튼을 제공합니다.
...
- 헤더 → DAY 탭 → 날짜 소개/액션 → sticky 한글 지도 → 오늘의 동선 → 카테고리 카드 → 하단 내비게이션
...
- 헤더와 DAY 탭, sticky 한글 지도, 카테고리 색상 카드, 하단 4개 메뉴, BottomSheet 상세 화면의 구조와 타이포그래피를 유지해줘.
...
| 일정 | 헤더 아래 고정형 DAY 1~4 탭·지도, 장소 목록 스크롤, 맨 위로 이동, 실제 방문 수, 대체 후보 토글, 장소 추가 |
...
중요: 현재 저장소의 교토·고베 앱 디자인을 공통 기준으로 그대로 재사용해줘. 새 여행마다 별도 초록색 dashboard나 다른 헤더/카드/CSS를 만들지 말고, 기존 `src/Prototype.tsx`, `src/prototype.css`, `MobileScroll`, `BottomSheet`를 사용해줘. 헤더·DAY 탭·sticky 지도·카테고리 색상 카드·하단 내비게이션의 구조와 반응형 규칙을 유지하고, 실제 모바일 스크린샷으로 잘림·겹침을 검수해줘.
```
...
12. 기존 교토·고베 화면과 비교해 새 여행의 header, DAY 탭, sticky 지도,
