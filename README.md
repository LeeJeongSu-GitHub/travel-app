...
여행지별 폴더를 하나의 모바일 우선 정적 웹앱 허브에서 관리합니다. 현재는 `kyoto-kobe-trip/`의 교토·고베 일정과 `guam-trip/`의 괌 3박 4일 일정이 들어 있습니다.
...
| **현재 여행** | `guam-trip/` · 괌 임산부 동반 힐링 3박 4일 |
...
로컬 괌         http://localhost:5173/guam-trip/
...
현재 기준 여행은 교토·고베와 괌 3박 4일 일정입니다. 괌 일정은 `guam-trip/`에 독립적으로 보관됩니다.
...
- 괌 태교여행 앱은 `guam-trip/`에 독립적으로 보관되며, 날짜별 투몬·북부·남부 동선과 대체 식당을 제공합니다.
